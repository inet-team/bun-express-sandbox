...
* `GET /api/v1/news?category=it-business` → Filter news by category slug (e.g., `'it-business'` for 'ธุรกิจไอที').
* `GET /api/v1/news?type=ไทย` → Filter news by content type (`ไทย` or `ต่างประเทศ`).
